...
function MonthView(element, calendar) {
	...
	function render(date) {
		...
		t.end.add((7 - t.end.weekday()) % 7, 'days'); 
		t.end = t.skipHiddenDays(t.end, -1, true); 
		var rowCnt = Math.ceil( 
			t.end.diff(t.start, 'weeks', true) 
		);
		...
		eventElement.draggable({
			...
			stop: function(ev, ui) {
				...
				if (revert) { 
					...
					var eventStart = event.start.clone().add(dayDelta, 'days'); 
					var snapTime;
					var snapIndex;
					if (!allDay) {
		...
		handle.mousedown(function(ev) {
			...
			hoverListener.start(function(cell, origCell) {
				if (cell) {
					...
					eventEnd = getEventEnd(event).add(dayDelta, 'days'); 
					if (dayDelta) {
